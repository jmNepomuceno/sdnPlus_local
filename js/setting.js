...
$(document).ready(function(){
    ...
    $('#add-doctor-btn').on('click' , function(event){
      let form_input = [$('#input-lname') , $('#input-mname') , $('#input-fname') , $('#input-mnum')]
      let validity = false
      for(let elem of form_input){
        validity = (elem.val() != "" && elem.val() != null) ? true : false
      }
         ...
         const data = {
          ...
          $.ajax({
              ...
              success: function(response) {
                ...
                $('#myModal-prompt #yes-modal-btn-incoming').text('Yes');
                $("#myModal-prompt #yes-modal-btn-incoming").css("display", "none")
                $("#myModal-prompt #ok-modal-btn-incoming").css("margin-right", "0")
          ...
          $('#modal-body').text('Fill out the input fields')
          $('#modal-title-incoming').text('Warning')
          $('#modal-icon').attr('class', 'fa-solid fa-circle-exclamation');
          $("#myModal-prompt #yes-modal-btn-incoming").css("display", "none")
          $("#myModal-prompt #ok-modal-btn-incoming").css("margin-right", "0")
          prompt_modal.show()
        }
    ...
    $(document).on('click', '.remove-doctor-btn', function(event){
      ...
      $.ajax({
          ...
          method: "POST",
          success: function(response) {
            $('#modal-body').text('Removed Successfully')
            $('#modal-icon').attr('class', 'fa-solid fa-circle-exclamation');
            $("#myModal-prompt #ok-modal-btn-incoming").css("margin-right", "0")
  ...
  $('#notif-sub-div').on('click' , function(event){
    ...
    // List of input IDs to apply the event listener
    const inputs = ['input-lname', 'input-fname', 'input-mname'];

    // Function to validate input
    function validateInput(event) {
        const regex = /^[a-zA-Z\s\-]*$/;  // Letters, spaces, and hyphens
        if (!regex.test(event.target.value)) {
            event.target.value = event.target.value.replace(/[^a-zA-Z\s\-]/g, '');  // Remove invalid characters
        }
    }

    // Loop through each input and apply the event listener
